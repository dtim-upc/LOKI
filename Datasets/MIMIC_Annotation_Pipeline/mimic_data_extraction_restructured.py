...
# Pre-pass: build prescription hadm_ids using only (subject_id, hadm_id)
# This is a separate pass because prescriptions.csv has a different row count
# than diagnoses_icd.csv / discharge.csv, so it can't be zip-iterated.
pres_hadm_dict = {}  # {subject_id: set of hadm_ids}
print("Pre-scanning prescriptions.csv for admission coverage...")
for pres_chunk in pd.read_csv(
    f"{DATA_DIR}/prescriptions.csv",
    usecols=["subject_id", "hadm_id"],
    chunksize=chunk_size,
    low_memory=False,
):
    for sid, group in pres_chunk.groupby("subject_id"):
        if sid not in pres_hadm_dict:
            pres_hadm_dict[sid] = set()
        pres_hadm_dict[sid].update(group["hadm_id"].dropna().values)
print(f"  ✓ Prescription coverage built for {len(pres_hadm_dict)} patients\n")
...
for diag_chunk, discharge_chunk in zip(diag_reader, discharge_reader):
    ...
    # Validity requires every diagnosis admission to have:
    #   1. a discharge note, AND
    #   2. at least one prescription row (so medication.csv will be non-empty)
    for sid in diag_hadm_dict:
        if sid not in valid_patients:
            ...
            pres_ids = pres_hadm_dict.get(sid, set())
            if (len(diag_ids) > 0
                    and diag_ids.issubset(note_ids)
                    and diag_ids.issubset(pres_ids)):
