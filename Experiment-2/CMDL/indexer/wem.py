...
class WEM():
  def __init__(self, ft_model_path, dim):
    ...
    # Prefer gensim-saved format (fast reload). If only Facebook .bin exists, convert it once.
    gensim_path = ft_model_path.replace('.bin', '.gensim') if ft_model_path.endswith('.bin') else ft_model_path + '.gensim'

    if os.path.exists(gensim_path):
      tqdm.write(f'  Using cached gensim format: {gensim_path}')
      self.ft_model = FastTextKeyedVectors.load(gensim_path, mmap='r')
    elif ft_model_path.endswith('.bin') and os.path.exists(ft_model_path):
      tqdm.write(f'  Loading Facebook .bin format (first time -- will convert to gensim for faster future loads)')
      kv = load_facebook_vectors(ft_model_path)
      tqdm.write(f'  Saving gensim format to: {gensim_path}')
      kv.save(gensim_path)
      self.ft_model = kv
    else:
      self.ft_model = FastTextKeyedVectors.load(ft_model_path, mmap='r')

  def __call__(self, word_list):
    assert self.ft_model is not None
    assert word_list is not None
    vectors = [self.ft_model[w] for w in word_list]
...
class WEMIndexer(object):
  ...
  def commit_index(self):
    ...
    with open(self.index_name + '-idmap', 'w', encoding='utf-8') as f:
      csvf = csv.writer(f)
  ...
  def load_index(self):
    ...
    with open(self.index_name + '-idmap', 'r', encoding='utf-8') as f:
      csvf = csv.reader(f)
      for row in csvf:
