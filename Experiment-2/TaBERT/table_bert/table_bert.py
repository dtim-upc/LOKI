    ...
    def from_pretrained(
        ...
        **kwargs
        ...
        if state_dict is None:
            state_dict = torch.load(model_name_or_path, map_location="cpu", weights_only=False)
        ...
        if TRANSFORMER_VERSION == TransformerVersion.TRANSFORMERS:
            old_key_to_new_key_names: List[(str, str)] = []
            for key in state_dict:
                if key.endswith('.predictions.bias') or key.endswith('.prediction.bias'):
                    new_key = key.rsplit('.bias', 1)[0] + '.decoder.bias'
                    old_key_to_new_key_names.append((key, new_key))

            for old_key, new_key in old_key_to_new_key_names:
                state_dict[new_key] = state_dict[old_key]

        # remove pooler keys that exist in old checkpoints but not in modern BertForMaskedLM
        model_keys = set(model.state_dict().keys())
        keys_to_remove = [k for k in list(state_dict.keys()) if k not in model_keys and 'pooler' in k]
