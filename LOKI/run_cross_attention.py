...
def _bootstrap_training_hf_assets(model_name: str) -> None:
...
def main():
    ...
    AVAILABLE_MODELS = {
        ...
        "agent-moderncolbert": "lightonai/Agent-ModernColBERT",
        ...
        "modernbert-bio-base": "almanach/ModernBERT-bio-base",
        "modernbert-bio-large": "almanach/ModernBERT-bio-large",
    ...
    parser.add_argument("--triplet_strategy", type=str, default="full",
                        ...
                        help="Strategy for generating triplets: 'full' (all pos×neg combinations), "
                             ...
                             "'primary_only' (only 1 pos × 1 neg per example)")
    ...
    parser.add_argument("--diversity_weight", type=float, default=0.0,
                        help="Relative weight for cross-row diversity loss (bidirectional only, will be normalized with other weights)")
    parser.add_argument("--direct_attention_loss_weight", type=float, default=0.1,
                        help="Relative weight for direct sparse-attention anti-collapse loss (bidirectional only, will be normalized with other weights). Outer multiplier that activates the three direct-attention sub-losses below; set to 0.0 to disable.")
    parser.add_argument("--direct_attention_diversity_weight", type=float, default=1.0,
                        help="Subweight for pairwise query-distribution diversity inside direct attention loss")
    parser.add_argument("--direct_attention_hub_weight", type=float, default=0.05,
                        help="Subweight for hub-key concentration penalty inside direct attention loss (prevents all rows attending to the same sentences)")
    parser.add_argument("--direct_attention_entropy_weight", type=float, default=0.1,
                        help="Subweight for sparse-attention entropy floor inside direct attention loss (penalises collapsed AND uniform attention rows)")
    parser.add_argument("--direct_attention_entropy_floor_ratio", type=float, default=0.5,
                        help="Fraction of log(top_k) used as sparse-attention entropy floor")
    parser.add_argument("--forward_attention_loss_weight", type=float, default=0.0,
                        help="Relative weight for forward row-to-sentence attention anti-collapse loss")
    parser.add_argument("--pair_mil_loss_weight", type=float, default=0.0,
                        help="Relative weight for optional MIL bag loss over raw pair_scores")
    parser.add_argument("--pair_mil_positive_margin", type=float, default=0.2,
                        help="Minimum desired top-k raw pair score strength for positive table-text pairs")
    parser.add_argument("--pair_mil_negative_margin", type=float, default=0.05,
                        help="Maximum tolerated top-k raw pair score strength for negative table-text pairs")
    parser.add_argument("--pair_mil_sparsity_weight", type=float, default=0.0,
                        help="Subweight for optional MIL sparsity pressure over pair_scores")
    parser.add_argument("--pair_mil_hub_weight", type=float, default=0.0,
                        help="Subweight for optional MIL hub concentration penalty over pair_scores")
    ...
    parser.add_argument("--use_attention_distillation", action="store_true", default=True,
                        help="[DEPRECATED for LeWM mode] Enable attention distillation from frozen encoder to preserve zero-shot row-sentence alignments during training")
    ...
    parser.add_argument("--teacher_temperature", type=float, default=0.5,
                        help="Temperature for teacher (frozen encoder) pair similarities. Lower = sharper distribution")
    ...
    parser.add_argument("--teacher_hub_centering", action="store_true", default=True,
                        help="Subtract per-sentence mean across rows before teacher distillation softmax")
    parser.add_argument("--no_teacher_hub_centering", dest="teacher_hub_centering", action="store_false",
                        help="Disable hub-suppressed teacher distillation")

    # SIGReg and Sinkhorn regularization arguments (anti-collapse structural priors)
    parser.add_argument("--sigreg_weight", type=float, default=0.15,
                        help="Relative weight for SIGReg (spectral invariance + covariance) loss on contextualized "
                             "embeddings. Prevents mode-collapse in the representation space produced by cross-attention. "
                             "Recommended: 0.1-0.2 (will be normalized with other weights)")
    parser.add_argument("--sigreg_target_std", type=float, default=1.0,
                        help="Target per-dimension standard deviation for SIGReg variance term. "
                             "Dimensions with std below this are penalized. Default: 1.0")
    parser.add_argument("--sigreg_num_proj", type=int, default=1024,
                        help="Number of random projection directions for Epps-Pulley SIGReg.")
    parser.add_argument("--sigreg_knots", type=int, default=17,
                        help="Number of quadrature knots for Epps-Pulley integral.")
    parser.add_argument("--sinkhorn_weight", type=float, default=0.1,
                        help="Relative weight for Sinkhorn marginal constraint on attention matrices. "
                             "Prevents hub sentences by constraining column marginals toward uniform distribution. "
                             "Recommended: 0.1-0.2 (will be normalized with other weights)")
    ...
    parser.add_argument("--attention_type", type=str, default="standard",
                        ...
                        help="Type of attention mechanism: standard (regular softmax — default, no top-k entropy ceiling so the entropy/diversity regularisers can actually reshape attention), top_k_sparse (sparse attention), windowed (local attention), threshold (semantic filtering)")
    ...
    # Outer gate — applied in BidirectionalCrossAttention after the inner attention module returns
    # (forward_output_gate / reverse_output_gate).  Controls whether the bidirectional context
    # update is suppressed at the cross-attention boundary.
    parser.add_argument("--use_gated_attention", action="store_true", default=True,
                        help="Enable outer query-dependent gate on attention outputs in BidirectionalCrossAttention")
    parser.add_argument("--no_gated_attention", dest="use_gated_attention", action="store_false",
                        help="Disable outer gated attention (BidirectionalCrossAttention level)")
    parser.add_argument("--gated_attention_mode", type=str, default="vector",
                        choices=["scalar", "vector"],
                        help="Gate shape for both outer and inner gates: scalar or vector")
    parser.add_argument("--gated_attention_hidden_dim", type=int, default=0,
                        help="Hidden dim for 2-layer gating MLP (0 => single linear, shared by both gates)")
    parser.add_argument("--gated_attention_dropout", type=float, default=0.0,
                        help="Dropout on gate values (after sigmoid, shared by both gates)")
    parser.add_argument("--gated_attention_init_bias", type=float, default=6.0,
                        help="Initial bias for gate logits (sigmoid(6.0)≈0.998 → near pass-through at init so cross-attention contributes immediately; init_bias=0.0 was tried and stalled training, shared by both gates)")

    # Inner gate — applied inside TopKSparseAttention / WindowedCrossAttention / ThresholdAttention
    # before the output is returned to BidirectionalCrossAttention.  Enables a second independent
    # gate at the sparse-attention level (double-gate when combined with --use_gated_attention).
    parser.add_argument("--use_inner_gate", action="store_true", default=True,
                        help="Enable inner gate inside attention modules (TopKSparseAttention etc.); combine with --use_gated_attention for double-gate")
    parser.add_argument("--no_inner_gate", dest="use_inner_gate", action="store_false",
                        help="Disable inner gate (default; only outer gate is active when --use_gated_attention is set)")
    ...
    parser.add_argument("--sparse_top_k", type=int, default=20,
                        help="Number of top connections to keep in sparse attention. Raised from 5 to 20 to lift the softmax-over-top-k entropy ceiling from log(5)=1.61 to log(20)=3.00, giving the entropy regulariser room to actually shape attention.")
    ...
    parser.add_argument("--row_sent_test_file", type=str, default=str(_DATASETS_DIR / "mimic" / "test_row_level.json"),
                        help="Path to test dataset for row-sentence evaluation")
    parser.add_argument("--row_sent_annotation_file", type=str, default=str(_DATASETS_DIR / "mimic" / "Annotated_Test.json"),
    ...
    parser.add_argument("--enable_attention_diagnostics", action="store_true", default=True,
                        help="Report sparse attention collapse diagnostics during row-sentence evaluation")
    ...
    parser.add_argument("--attention_diagnostic_examples", type=int, default=3,
                        help="Number of row-sentence examples to sample for attention collapse diagnostics")
    ...
    if args.init_method_params:
        try:
            ...
            print(f"❌ Error parsing init_method_params JSON: {e}")
    ...
    if args.use_header_conditioning and not args.use_bidirectional:
        print("❌ --use_header_conditioning currently supports bidirectional LOKI only.")
        sys.exit(1)
    if args.use_cell_level_matching and not args.use_bidirectional:
        print("❌ --use_cell_level_matching currently supports bidirectional LOKI only.")
        sys.exit(1)
    ...
    if args.use_bidirectional and args.aggregation_method in ["mean", "top_k_sum", "top_k_mean", "weighted_top_k",
                                                              ...
                                                              "entropy_regularized"]:
        print(
            f"⚠️  Switching aggregation method from '{args.aggregation_method}' to 'top_k_pairs' for bidirectional architecture")
    ...
    if args.use_bidirectional and args.loss_type in ["id_cached_triplet", "enhanced_triplet"]:
        print(
            f"⚠️  Switching loss type from '{args.loss_type}' to 'bidirectional_triplet' for bidirectional architecture")
        args.loss_type = "bidirectional_triplet"

    # Resolve encoder training setting from mutually exclusive arguments
    enable_encoder_training = args.enable_lora and not args.disable_lora
    ...
    if (args.use_unsloth and args.unsloth_qlora and not args.unsloth_full_finetuning
        ...
        print("\n" + "=" * 70)
        print("⚠️  CONFIGURATION NOTE: QLoRA + Gradual Unfreezing")
        print("=" * 70)
        ...
        print("")
        print("   Options:")
        print("   1. Keep QLoRA (recommended): All LoRA adapters train from epoch 0")
        print("      - Memory efficient (~1-5% of parameters)")
        print("      - Good performance with proper hyperparameters")
        print("")
        print("   2. Use gradual unfreezing without QLoRA:")
        print("      --no_unsloth OR --unsloth_full_finetuning")
        print("      - Full control over which layers to unfreeze")
    ...
    if enable_encoder_training and use_cache:
        print(
            "⚠️  Encoder fine-tuning (--enable_lora) detected: disabling cache for training so encoder updates affect embeddings")
        use_cache = False
    # Stage 0 cache logic is now handled in train.py for more sophisticated mode detection
    if use_cache:
        print("✅ Cache enabled with --use_cache flag")
    else:
        print("🔄 Cache disabled for training")

    try:
        # Get the actual model name from the dictionary using the key
        model_name = AVAILABLE_MODELS[args.model_key]
        _bootstrap_training_hf_assets(model_name)
        ...
        if use_unsloth_loading and "/" not in model_name and not os.path.exists(model_name):
            ...
            print(f"   ℹ️ Auto-resolving short model name to '{unsloth_model_name}' for Unsloth")

        if use_unsloth_loading:
            # Use Unsloth for 2x faster training
            print("\n" + "=" * 60)
            print("🦥 UNSLOTH MODE: Loading encoder with optimized kernels")
            print("=" * 60)
            print_unsloth_status()
            ...
            if args.override_max_seq_length > 0:
                print(f"📏 Overriding max_seq_length: {detected_max_seq_length} → {args.override_max_seq_length}")
            ...
            try:
                ...
                print("✅ Unsloth encoder loaded successfully!")
                ...
                if args.unsloth_qlora and not args.unsloth_full_finetuning:
                    ...
                    # =====================================================================
                    print("\n   📊 Encoder PEFT/LoRA Parameter Breakdown:")
                    ...
                    if encoder_trainable == encoder_total:
                        print("\n   ⚠️ WARNING: All encoder params are trainable!")
                        ...
                        print(
                            f"\n   ✅ QLoRA working correctly! Only {encoder_trainable / encoder_total * 100:.2f}% params trainable")
                ...
                if enable_encoder_training:
                    print(
                        "   ⚠️ WARNING: QLoRA adapters NOT attached - encoder will be fully trainable (100%) due to enable_lora flag")
                else:
                    print("   ℹ️ Note: Encoder will be properly frozen (0% trainable) during standard loading.")
        ...
        if not use_unsloth_loading:
            ...
            if args.use_unsloth and not UNSLOTH_AVAILABLE:
                print("⚠️ Unsloth requested but not available. Using standard loading.")

            resolved_encoder_model_name, encoder_model_source = ensure_repo_local_hf_snapshot(
                model_name,
                allow_online=True,
            )
            print(
                f"   HF snapshot ready for standard loading: {resolved_encoder_model_name} "
                f"({encoder_model_source})"
            )

            # Warn that QLoRA is not being used
            if args.unsloth_qlora and args.use_unsloth:
                print("\n" + "=" * 70)
                print("⚠️  IMPORTANT: QLoRA was requested but Unsloth loading failed!")
                print("=" * 70)
        ...
        # Ensure a Pooling module is present in the SentenceTransformer pipeline (critical for ColBERT/PyLate models)
        has_pooling = any("Pooling" in type(m).__name__ for m in sentence_encoder._modules.values())
        if not has_pooling:
            print("⚠️ No Pooling module detected in encoder pipeline. Appending mean pooling layer...")
            from sentence_transformers import models as sbert_models
            transformer_module = next(iter(sentence_encoder._modules.values()))
            hidden_size = getattr(transformer_module.auto_model.config, "hidden_size", 768) if hasattr(transformer_module, "auto_model") else 768
            pooling_module = sbert_models.Pooling(embedding_dimension=hidden_size, pooling_mode="mean")
            
            # Safely rebuild SentenceTransformer with the new pooling layer
            pipeline_modules = list(sentence_encoder._modules.values())
            pipeline_modules.append(pooling_module)
            sentence_encoder = SentenceTransformer(modules=pipeline_modules)
            print(f"✅ Appended mean pooling module (hidden_size={hidden_size})")

        # Apply max_seq_length override for ALL loading paths (Unsloth and standard).
        # For the Unsloth path the override was already threaded through detected_max_seq_length,
        # but FastSentenceTransformer may not honour it faithfully (e.g. tokenizer model_max_length
        # can survive from the model config).  Explicitly enforcing it here is the safety net.
        if args.override_max_seq_length > 0:
            ...
            print(f"📏 Enforcing max_seq_length override: {native_max_seq} → {args.override_max_seq_length}")

        # Get true embedding dimension by executing a reliable dummy forward pass
        print("🔍 Doing dummy forward pass to extract exact native embedding dimension...")
        try:
            dummy_emb = sentence_encoder.encode(["Test"], convert_to_tensor=True)
            native_embedding_dim = dummy_emb.shape[-1]
            print(f"✅ Extracted true native embedding dimension from forward pass: {native_embedding_dim}")
        except Exception as e:
            print(f"⚠️ Error extracting embedding dimension via forward pass: {e}. Falling back to get_sentence_embedding_dimension()")
            native_embedding_dim = sentence_encoder.get_sentence_embedding_dimension() or 768
        ...
        if args.override_embedding_dim > 0:
            ...
            if target_dim >= native_embedding_dim:
                print(f"⚠️ override_embedding_dim ({target_dim}) >= native dim ({native_embedding_dim}), ignoring override.")
            else:
                ...
                try:
                    ...
                    if actual_dim == target_dim:
                        ...
                        print(f"✅ Matryoshka truncation: {native_embedding_dim} → {target_dim} (model supports it)")
                    else:
                        # Model didn't truncate — clear and use projection instead
                        sentence_encoder.truncate_dim = None
                        embedding_dim = target_dim
                        print(f"📐 Model does not support Matryoshka at dim={target_dim}. "
                              f"A linear projection layer ({native_embedding_dim} → {target_dim}) will be added.")
                except Exception as e:
                    sentence_encoder.truncate_dim = None
                    embedding_dim = target_dim
                    print(f"📐 Matryoshka test failed ({e}). Using projection layer: {native_embedding_dim} → {target_dim}")
        ...
        if args.max_train_examples > 0 and len(train_examples) > args.max_train_examples:
            ...
            print(
                f"⚡ Sampled {len(train_examples)} training examples (deterministic, seed={args.seed if hasattr(args, 'seed') else 42})")
        ...
        if effective_max_eval == 0 and args.max_train_examples > 0:
            ...
            print(f"📊 Auto-scaling validation set to {effective_max_eval} examples (10% of training subset)")
        ...
        if effective_max_eval > 0 and len(eval_examples) > effective_max_eval:
            ...
            print(f"⚡ Sampled {len(eval_examples)} evaluation examples (deterministic)")
        ...
        if args.use_bidirectional:
            model = BidirectionalTableTextModel(
                ...
                # Gated attention (outer gate) + inner gate + temperature disable
        ...
        if args.use_compile and TORCH_COMPILE_AVAILABLE:
            print("\n" + "=" * 60)
            print("⚡ TORCH.COMPILE: Optimizing custom attention modules")
            print("=" * 60)
            ...
            print("⚠️ torch.compile() requested but not available (requires PyTorch 2.0+)")
        ...
        print(f"\n📊 Model Parameter Statistics (Initial):")
        ...
        if args.use_unsloth and args.unsloth_qlora and not args.unsloth_full_finetuning:
            ...
            print(f"  ℹ️  Note: Final trainable % will be set by train.py based on training mode")
        ...
        print(f"  Embedding caching: {'✅ Enabled' if use_cache else '❌ Disabled'}")
        print(f"  Triplet batching: {'🔄 Mixed examples' if args.mix_examples else '🔒 Isolated examples'}")
        ...
        print(f"\n  📊 Loss Component Weights (will be normalized):")
        ...
        if args.use_bidirectional:
            ...
            print(f"    Direct attention weight: {args.direct_attention_loss_weight}")
            print(f"    Forward attention weight: {args.forward_attention_loss_weight}")
            print(f"    Pair MIL weight: {args.pair_mil_loss_weight}")
            print(f"    Pair weight: {args.pair_loss_weight}")
            if args.sigreg_weight > 0:
                print(f"    SIGReg weight: {args.sigreg_weight} (target_std={args.sigreg_target_std})")
            if args.sinkhorn_weight > 0:
                print(f"    Sinkhorn weight: {args.sinkhorn_weight}")
        print(f"\n  ⚙️ Loss Parameters:")
        ...
        if args.use_bidirectional:
            ...
            print(f"    Teacher hub centering: {args.teacher_hub_centering}")
        ...
        if args.use_bidirectional:
            print(f"\n  🔄 Bidirectional-Specific Configuration:")
            print(f"    Pair margin: {args.pair_margin}")
            print(f"    Pair score method: {args.pair_score_method}")
            print(f"    Share attention weights: {args.share_attention_weights}")
            print(f"    Join path extraction: {args.extract_join_paths}")
        ...
        config_dict['diversity_weight'] = args.diversity_weight if args.use_bidirectional else 0.0
        config_dict['direct_attention_loss_weight'] = args.direct_attention_loss_weight if args.use_bidirectional else 0.0
        config_dict['direct_attention_diversity_weight'] = args.direct_attention_diversity_weight if args.use_bidirectional else 0.0
        config_dict['direct_attention_hub_weight'] = args.direct_attention_hub_weight if args.use_bidirectional else 0.0
        config_dict['direct_attention_entropy_weight'] = args.direct_attention_entropy_weight if args.use_bidirectional else 0.0
        config_dict['direct_attention_entropy_floor_ratio'] = args.direct_attention_entropy_floor_ratio if args.use_bidirectional else 0.0
        config_dict['forward_attention_loss_weight'] = args.forward_attention_loss_weight if args.use_bidirectional else 0.0
        config_dict['pair_mil_loss_weight'] = args.pair_mil_loss_weight if args.use_bidirectional else 0.0
        config_dict['pair_mil_positive_margin'] = args.pair_mil_positive_margin if args.use_bidirectional else 0.0
        config_dict['pair_mil_negative_margin'] = args.pair_mil_negative_margin if args.use_bidirectional else 0.0
        config_dict['pair_mil_sparsity_weight'] = args.pair_mil_sparsity_weight if args.use_bidirectional else 0.0
        config_dict['pair_mil_hub_weight'] = args.pair_mil_hub_weight if args.use_bidirectional else 0.0
        config_dict['teacher_hub_centering'] = args.teacher_hub_centering if args.use_bidirectional else False
        config_dict['sigreg_weight'] = args.sigreg_weight if args.use_bidirectional else 0.0
        config_dict['sigreg_target_std'] = args.sigreg_target_std
        config_dict['sinkhorn_weight'] = args.sinkhorn_weight if args.use_bidirectional else 0.0
        ...
        config_dict['enable_attention_diagnostics'] = args.enable_attention_diagnostics
        config_dict['attention_diagnostic_examples'] = args.attention_diagnostic_examples
        ...
        trained_model = train_with_id_based_triplets(
            ...
            direct_attention_entropy_floor_ratio=args.direct_attention_entropy_floor_ratio if args.use_bidirectional else 0.0,
        ...
        print(f"\n✅ Training completed! The returned model is the BEST performing model from training.")
        print(f"📁 Check the training logs above to see which epoch achieved the highest validation accuracy.")
        ...
        if args.test_file:
            ...
            if args.max_eval_examples > 0 and len(test_examples) > args.max_eval_examples:
                ...
                print(f"⚡ Sampled {args.max_eval_examples} test examples for final evaluation (deterministic)")
        ...
        if args.do_visualize and test_examples:
            ...
            print(f"✅ CONFIRMED: Using BEST model checkpoint from training")
            print(f"📁 Best model path: {best_model_path}")
            print(f"🔧 Using aggregation method: {args.aggregation_method}")
            ...
            for idx in example_indices:
                if idx < len(test_examples):
                    print(f"🔬 Creating comprehensive analysis for example {idx} using BEST model...")
                    ...
                    if rows and sentences:
                        print(f"🔬 Generating step-by-step diagnostics for example {idx}...")
                        ...
                        print(f"  ⚠️ Could not extract rows/sentences for example {idx}")
                        continue
            ...
            if args.skip_four_stage_viz:
                ...
                print(f"\n🎯 Generating 4-STAGE EVALUATION INTEGRITY ANALYSIS...")
                print(f"✅ This shows Stage 0→1→2→3 progression for research integrity")
                try:
                    ...
                    print(f"✅ 4-stage analysis completed and saved to: {four_stage_output_dir}")
                    ...
                    print(f"⚠️ Could not generate 4-stage analysis: {e}")
        ...
        if args.do_clean_analysis and test_examples:
            print(f"\n🧪 Running CLEAN step-by-step analysis to fix inconsistencies...")
            print(f"✅ This system computes everything from scratch using single forward pass")
            print(f"🔧 Using refinement setting: {args.use_refinement}")
                ...
                if results:
                    ...
                    if not args.use_refinement and 'comparison_status' in results:
                        ...
                        if comparison == "DIFFERENT":
                            print(f"               ⚠️  BUG DETECTED: Should be identical when refinement=False")
                else:
                    print(f"  Example {idx}: Skipped (missing data)")

            print(f"🔍 Detailed results saved in: {output_dir}/clean_diagnostics/")
            print(f"📈 Check individual heatmaps and summary.json files for each example")
